...
const GuestDashboard = ({ stats, age, experience, session }: GuestDashboardProps) => {
  ...
  const renderRadarChart = (subset: StatsType, index: number) => {
    ...
    const values = labels.map((l) => subset[l.toLowerCase()]?.total > 0 ? (subset[l.toLowerCase()].correct / subset[l.toLowerCase()].total) * 100 : 0);
    return (
      <Card key={index} sx={{ p: 2 }}>
        <CardContent>
          <Typography variant="h6" align="center">Performance by Topic</Typography>
          <Box sx={{ height: 250, position: "relative" }}>
            <Radar data={{ labels, datasets: [{ label: "Performance", data: values, backgroundColor: "rgba(54,162,235,0.2)", borderColor: "rgba(54,162,235,1)", borderWidth: 2 }] }} options={{ responsive: true, maintainAspectRatio: false, scales: { r: { min: 0, max: 100 } } }} />
          </Box>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: "auto", p: 3 }}>
      <Typography variant="h4" align="center" gutterBottom>🔒 SeQG - Your Cybersecurity Assessment Results</Typography>
      ...
      <Button variant="contained" onClick={handleReturn}>Return to Main Screen</Button>

      <Modal open={showModal}><Paper sx={{ p: 3, textAlign: 'center', position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }}><Typography variant="h6">Are you still viewing?</Typography><Typography>Returning in {countdown} sec...</Typography><Button variant="contained" onClick={handleStay}>I'm still here!</Button></Paper></Modal>
    </Box>
  );
};
